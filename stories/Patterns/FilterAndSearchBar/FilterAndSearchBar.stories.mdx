...
- A multi-select is toggled open or closed:
  ```
  $(document).on('multiSelectToggle', (e) => {
    ...
  })
  ```
- A checkbox in the multi-select is toggled on or off:
  ```
  $(document).on('multiSelectInputToggle', (e) => {
    ...
  })
  ```
- A Search chip is removed:
  ```
  $(document).on('filterSearchChipRemoval', (e) => {
    ...
  })
  ```
- The Search clear all link is clicked:
  ```
...
The event data for each of the custom events contain specific data.
