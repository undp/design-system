...
export default {
  ...
  argTypes: {
    locale: {
      control: {
        type: "select",
        options: ["english", "spanish", "french", "burmese", "japanese"],
      },
    },
  },
};
...
const Template = (args, { globals: { locale, accent } }) => {
  ...
  const { groupcontentdata, headertext, label, whatwedoheader, whatwedodescription, img } = getGlobalHomepageData(locale);
  return (
    <GlobalHomePageRTL
    locale={locale}
...
GlobalHomePageStory.parameters = {
  docs: {
    description: {
      ...
      <h3>Overview</h3>
      <p>A global page has all the content listed. This helps the user to get all the data in one place. Mock Up: https://undp-design-system.azurewebsites.net/</p>
      ...
      <p>Initialize navigation, language switcher, accordion, statsHover, swiper by calling below mentioned functions.when visible on screen</p>
      <ul>
      ...
      <h4>Add following external libraries</h4>
      <ul>
