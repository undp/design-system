...
// Swiper Slider
export const swiper = (selector, options) => {
  ...
  swiperSelector.each(function (index, element) {
  ...
  // let swiperParallax = $(element).data('swiper-parallax') ? $(element).data('swiper-parallax') : false;
    ...
    let defaults = {
      ...
      slidesPerView: $(element).data('swiper-slides-view') ? $(element).data('swiper-slides-view') : 1,
    ...
    const getTrackSize = () => {
      return $(element).find('.swiper-scrollbar').width();
    }

    // Get number of the slides
    const getSliderLength = () => {
      return $(element).find('.swiper-slide').length;
    }

    // Get the Slider Bound in case of perView is more than 1.
    const getSliderBound = () => {
      return (getSliderLength() - options.slidesPerView);
    }
        ...
        swiper.on('beforeDestroy', function () {
          ...
          if (events) {
            if (events.click.length > 0) {
              $(element).find('.swiper-wrapper').unbind('click');
            }
            if (events.mousemove.length > 0) {
              $(element).find('.swiper-wrapper').unbind('mousemove');
            }
          }
        });
        swiper.destroy();
    ...
    if (swiper !== undefined) {
      ...
      $(element).find('.swiper-wrapper').on('click', function (e) {
        e.preventDefault();
        ...
        updateArrow(e);
      }).find(ignoreClick).on('click', function (e){
        e.stopPropagation();
      });

      // Swiper slides mousemove eventlistner.
      $(element).find('.swiper-wrapper').on('mousemove', function (e) {
        e.preventDefault();
        updateArrow(e);
      });
    }
