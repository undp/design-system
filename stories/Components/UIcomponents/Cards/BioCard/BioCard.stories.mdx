...
<Meta 
  title="Components/UI components/Cards/Bio Cards" argTypes={{
  size: {
...
### Overview
A bio card showcases a contributor’s name, title, photo, and a short summary of their background and work. It also contains a CTA to open more information in a lightbox modal.
...
#### Default
The bio card consists of a user’s first and last names, title, a brief summary of their background/work, a profile picture, and a CTA that opens up a lightbox modal highlighting salient information about the user.

<Canvas>
  <Story name="Bio Cards">
    {(args, { globals: { locale } }) => {
